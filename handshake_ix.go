...
func ixHandshakeStage0(f *Interface, vpnIp iputil.VpnIp, hostinfo *HostInfo) {
	...
	err := f.handshakeManager.AddIndexHostInfo(hostinfo, hostinfo.networkID)
	if err != nil {
		f.l.WithError(err).WithField("vpnIp", vpnIp).WithField("networkID", hostinfo.networkID).
	...
	if ci == nil {
		f.l.WithError(err).WithField("vpnIp", vpnIp).WithField("networkID", hostinfo.networkID).
			WithField("handshake", m{"stage": 0, "style": "ix_psk0"}).Error("Connection state is nil")
		return
	}

	var rawCertificateNoKey []byte
	if f.lightHouse.amLighthouse {
		...
		f.signRequest[hostinfo.networkID] = time.Now().Unix()
		if err != nil {
			f.l.WithError(err).WithField("vpnIp", vpnIp).WithField("networkID", hostinfo.networkID).
				WithField("handshake", m{"stage": 0, "style": "ix_psk0"}).Error("Failed to certState")
		}
		if certState != nil {
			rawCertificateNoKey = certState.rawCertificateNoKey
		}
	} else {
		rawCertificateNoKey = ci.certState.rawCertificateNoKey
	}
	...
	h := header.Encode(make([]byte, header.Len), header.Version, header.Handshake, header.HandshakeIXPSK0, 0, 1, 0, 0, 0, 0, hostinfo.networkID)
...
func ixHandshakeStage1(f *Interface, addr *udp.Addr, packet []byte, h *header.H, relay uint8, relayIP *iputil.VpnIp) {
	...
	if f.lightHouse.amLighthouse {
		err := fmt.Errorf("Signing the certificates under progress")
		sendSignRequest := shallSendSignRequest(f, networkID)
		if f.certStateLock[networkID] == nil {
			f.certStateLock[networkID] = &sync.RWMutex{}
		}
		f.certStateLock[networkID].Lock()
		defer f.certStateLock[networkID].Unlock()
		certState, err = getrawCertState(networkID, f.relayIndex, f.sqlsecret, f.keysecret, sendSignRequest)
		f.signRequest[networkID] = time.Now().Unix()
		if err != nil {
			f.l.WithError(err).WithField("networkID", networkID).
				WithField("handshake", m{"stage": 0, "style": "ix_psk0"}).Error("Failed to get certState")
			return
		}
	}
	f.l.WithField("addr...", addr).Error("Received handshake stag1 packet")
	ci := f.newConnectionState(f.l, false, noise.HandshakeIX, []byte{}, 0, certState)
	...
	*/
	...
	if f.lightHouse.amLighthouse {
		ca, _, _, err = get_network_certs(networkID, f.sqlsecret)
		if err != nil {
			f.l.WithError(err).WithField("udpAddr", addr).
				WithField("networkID", networkID).
				Error("No Ca for this network")
			return
		}
	} else {
		ca = f.caFile
	}
	f.caPool[networkID], err = loadCAFromFile(f.l, ca)
	if err != nil {
		f.l.WithError(err).WithField("udpAddr", addr).
			WithField("networkID", networkID).
			Error("2. Unable to load CA")
		return
	}

	remoteCert, err := RecombineCertAndValidate(ci.H, hs.Details.Cert, f.caPool[networkID])
	if err != nil {
		f.l.WithError(err).WithField("udpAddr", addr).
	...
	f.l.WithField("vpnIp", vpnIp).WithField("udpAddr", addr).
		...
		WithField("relay", relay).WithField("relayip", relayIP).
		Info("Handshake message received")
	hostinfo.name = certName
	...
	nh := header.Encode(make([]byte, header.Len), header.Version, header.Handshake, header.HandshakeIXPSK0, hs.Details.InitiatorIndex, 2, 0, 0, 0, 0, hostinfo.networkID)
	...
	hostinfo.remotes = f.lightHouse.QueryCache(vpnIp, h.NetworkID)
	hostinfo.SetRemote(addr)
	hostinfo.CreateRemoteCIDR(remoteCert)

	// Only overwrite existing record if we should win the handshake race
	overwrite := vpnIp < f.myVpnIp
	existing, err := f.handshakeManager.CheckAndComplete(hostinfo, 0, overwrite, f, networkID)
	if err != nil {
		switch err {
		case ErrAlreadySeen:
			...
			if existing.SetRemoteIfPreferred(f.hostMap, addr) {
				...
				f.SendMessageToVpnIp(header.Test, header.TestRequest, vpnIp, []byte(""), make([]byte, 12, 12), make([]byte, mtu), networkID)
			}
			existing.Unlock()
			hostinfo.Lock()
			...
			if relay == 0 {
				...
				if err != nil {
					...
					f.l.WithField("vpnIp", existing.vpnIp).WithField("udpAddr", addr).
						WithField("handshake", m{"stage": 2, "style": "ix_psk0"}).WithField("cached", true).
						Info("2. Handshake message sent")
				}
			} else if f.certState != nil {
				hostinfo.relay = 1
				hostinfo.relayIP = relayIP
				err := f.SendRelay(header.RelayPacket, 0, msg, make([]byte, 12, 12),
					make([]byte, mtu), udp.Udp2ipInt(addr), udp.Ip2int(f.certState.certificate.Details.Ips[0].IP), 0, 0, hostinfo.networkID, hostinfo.relayIP)
				if err == nil {
					f.l.WithField("vpnIp", existing.vpnIp).WithField("udpAddr", addr).
						WithField("handshake", m{"stage": 2, "style": "ix_psk0"}).WithField("cached", true).
						WithField("header.RelayPacket", header.RelayPacket).
						Info("4. Handshake message sent")
				} else {
					f.l.WithField("vpnIp", existing.vpnIp).WithField("udpAddr", addr).
						WithField("handshake", m{"stage": 2, "style": "ix_psk0"}).WithField("cached", true).
						WithField("header.RelayPacket", header.RelayPacket).
						Info("Error while sending handshake through Relay", err)
				}
			}
			return
	...
	if relay == 0 {
		...
		f.l.Info("2. Have to Handshake...? Relay packet")
		hostinfo.relay = relay
		hostinfo.relayIP = relayIP
		err = f.SendRelay(header.RelayPacket, 0, msg, make([]byte, 12, 12),
			make([]byte, mtu), udp.Udp2ipInt(addr), udp.Ip2int(f.certState.certificate.Details.Ips[0].IP), 0, 0, hostinfo.networkID, hostinfo.relayIP)
	}
...
func ixHandshakeStage2(f *Interface, addr *udp.Addr, hostinfo *HostInfo, packet []byte, h *header.H) bool {
	...
	if ci.ready {
		...
		if hostinfo.SetRemoteIfPreferred(f.hostMap, addr) {
			// Send a test packet to ensure the other side has also switched to
			// the preferred remote
			f.SendMessageToVpnIp(header.Test, header.TestRequest, hostinfo.vpnIp, []byte(""), make([]byte, 12, 12), make([]byte, mtu), networkID)
		}
	...
	if vpnIp != hostinfo.vpnIp {
		...
		newHostInfo := f.getOrHandshake(hostinfo.vpnIp, networkID, true)
		newHostInfo.Lock()
		...
		hostinfo.remotes = f.lightHouse.QueryCache(vpnIp, networkID)
	...
	f.l.WithField("vpnIp", vpnIp).WithField("udpAddr", addr).
		...
		WithField("sentCachedPackets", len(hostinfo.packetStore)).
		Info("2. Handshake message received")

	hostinfo.name = certName
	hostinfo.remoteIndexId = hs.Details.ResponderIndex
	hostinfo.lastHandshakeTime = hs.Details.Time
	hostinfo.hsDuration = time.Since(hostinfo.handshakeStart).Milliseconds()
	...
	f.handshakeManager.Complete(hostinfo, f, networkID)
	hostinfo.handshakeComplete(f.l, f.cachedPacketMetrics)
	f.metricHandshakes.Update(duration)
	// Update RelayHostInfo
	f.UpdateRelayHostInfo()
	// Reset Handshakecounter
	hostinfo.HandshakeCounter = 0
