...
type LightHouse struct {
	...
	addrMap map[uint64]map[iputil.VpnIp]*RemoteList
	...
	handshakeTrigger chan<- NetworkIPPair
...
func NewLightHouse(l *logrus.Logger, amLighthouse bool, myVpnIpNet *net.IPNet, ips []iputil.VpnIp, interval int, nebulaPort uint32, pc *udp.Conn, punchBack bool, punchDelay time.Duration, metricsEnabled bool, networkID uint64) *LightHouse {
	ones, _ := myVpnIpNet.Mask.Size()
	h := LightHouse{
		amLighthouse: amLighthouse,
		myVpnIp:      iputil.Ip2VpnIp(myVpnIpNet.IP),
		myVpnZeros:   iputil.VpnIp(32 - ones),
		addrMap:      make(map[uint64]map[iputil.VpnIp]*RemoteList),
...
func (lh *LightHouse) Query(ip iputil.VpnIp, networkID uint64, f udp.EncWriter) *RemoteList {
	if !lh.IsLighthouseIP(ip) {
		lh.QueryServer(ip, networkID, f)
	}
	lh.RLock()
	if v, ok := lh.addrMap[networkID][ip]; ok {
		lh.RUnlock()
		return v
	}
	lh.RUnlock()
...
func (lh *LightHouse) QueryServer(ip iputil.VpnIp, networkID uint64, f udp.EncWriter) {
	if lh.amLighthouse {
		return
	}
	...
	for n := range lh.lighthouses {
		f.SendMessageToVpnIp(header.LightHouse, 0, n, query, nb, out, networkID)
	}
}

func (lh *LightHouse) QueryCache(ip iputil.VpnIp, networkID uint64) *RemoteList {
	lh.RLock()
	if v, ok := lh.addrMap[networkID][ip]; ok {
		lh.RUnlock()
		return v
	...
	return lh.unlockedGetRemoteList(ip, networkID)
}
...
func (lh *LightHouse) queryAndPrepMessage(vpnIp iputil.VpnIp, networkID uint64, f func(*cache) (int, error)) (bool, int, error) {
	lh.RLock()
	// Do we have an entry in the main cache?
	if v, ok := lh.addrMap[networkID][vpnIp]; ok {
...
func (lh *LightHouse) DeleteVpnIp(vpnIp iputil.VpnIp, networkID uint64) {
	...
	delete(lh.addrMap[networkID], vpnIp)
...
func (lh *LightHouse) AddStaticRemote(vpnIp iputil.VpnIp, toAddr *udp.Addr, networkID uint64) {
	lh.Lock()
	am := lh.unlockedGetRemoteList(vpnIp, networkID)
	am.Lock()
	defer am.Unlock()
	lh.Unlock()
...
func (lh *LightHouse) unlockedGetRemoteList(vpnIp iputil.VpnIp, networkID uint64) *RemoteList {
	am, ok := lh.addrMap[networkID][vpnIp]
	if !ok {
		am = NewRemoteList()
		if lh.addrMap[networkID] == nil {
			lh.addrMap[networkID] = make(map[iputil.VpnIp]*RemoteList)
		}
		lh.addrMap[networkID][vpnIp] = am
	}
	return am
}
...
func (lh *LightHouse) getLightHouseIPs() []iputil.VpnIp {
	lighthouseHosts := make([]iputil.VpnIp, MAX_LIGHTHOUSE_HOSTS)
	if lighthouseHosts == nil {
		return nil
	}
	i := 0
	for vpnIp := range lh.lighthouses {
		if i >= MAX_LIGHTHOUSE_HOSTS {
			break
		}
		lighthouseHosts[i] = vpnIp
		i++
	}
	return lighthouseHosts
}
...
func (lh *LightHouse) SendUpdate(f udp.EncWriter) {
	...
	for vpnIp := range lh.lighthouses {
		f.SendMessageToVpnIp(header.LightHouse, 0, vpnIp, mm, nb, out, lh.networkID)
	}
}
...
func (lhh *LightHouseHandler) HandleRequest(rAddr *udp.Addr, vpnIp iputil.VpnIp, networkID uint64, p []byte, w udp.EncWriter) {
	n := lhh.resetMeta()
	err := n.Unmarshal(p)
	if err != nil {
	...
	case NebulaMeta_HostQueryReply:
		lhh.handleHostQueryReply(n, vpnIp, networkID)

	case NebulaMeta_HostUpdateNotification:
		lhh.handleHostUpdateNotification(n, vpnIp, networkID)

	case NebulaMeta_HostMovedNotification:
	case NebulaMeta_HostPunchNotification:
		lhh.handleHostPunchNotification(n, vpnIp, w, networkID)
	}
}

func (lhh *LightHouseHandler) handleHostQuery(n *NebulaMeta, vpnIp iputil.VpnIp, networkID uint64, addr *udp.Addr, w udp.EncWriter) {
	...
	found, ln, err := lhh.lh.queryAndPrepMessage(iputil.VpnIp(n.Details.VpnIp), networkID, func(c *cache) (int, error) {
		...
		lhh.l.WithField("NebulaMeta_HostQueryReply reqVpnIp", udp.Int2ip(reqVpnIp)).WithField("networkID", networkID).Error("Replying NebulaMeta_HostQueryReply")
	...
	w.SendMessageToVpnIp(header.LightHouse, 0, vpnIp, lhh.pb[:ln], lhh.nb, lhh.out[:0], networkID)
	...
	w.SendMessageToVpnIp(header.LightHouse, 0, iputil.VpnIp(reqVpnIp), lhh.pb[:ln], lhh.nb, lhh.out[:0], networkID)
}
...
func (lhh *LightHouseHandler) handleHostQueryReply(n *NebulaMeta, vpnIp iputil.VpnIp, networkID uint64) {
	if !lhh.lh.IsLighthouseIP(vpnIp) {
		return
	}

	lhh.lh.Lock()
	am := lhh.lh.unlockedGetRemoteList(iputil.VpnIp(n.Details.VpnIp), networkID)
	am.Lock()
	lhh.lh.Unlock()
	...
	select {
	case lhh.lh.handshakeTrigger <- nip:
		lhh.l.Error("Going to trigger..", certVpnIp)
	default:
	}
}

func (lhh *LightHouseHandler) handleHostUpdateNotification(n *NebulaMeta, vpnIp iputil.VpnIp, networkID uint64) {
	if !lhh.lh.amLighthouse {
		if lhh.l.Level >= logrus.DebugLevel {
	...
	lhh.lh.Lock()
	am := lhh.lh.unlockedGetRemoteList(vpnIp, networkID)
	am.Lock()
	lhh.lh.Unlock()
...
func (lhh *LightHouseHandler) handleHostPunchNotification(n *NebulaMeta, vpnIp iputil.VpnIp, w udp.EncWriter, networkID uint64) {
	if !lhh.lh.IsLighthouseIP(vpnIp) {
		return
	}
	...
	if lhh.lh.punchBack {
		...
		go func() {
			...
			w.SendMessageToVpnIp(header.Test, header.TestRequest, queryVpnIp, []byte(""), make([]byte, 12, 12), make([]byte, mtu), networkID)
		}()
	}
}
