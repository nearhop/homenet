...
var headerBigEndianTests = []headerTest{{
	expectedBytes: []byte{0x54, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xa, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x9, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0},
	// 1010 0000
	H: &H{
		// 1111 1+2+4+8 = 15
		Version:        5,
...
func TestHeader_String(t *testing.T) {
	assert.Equal(
		...
		(&H{100, Test, TestRequest, 99, 98, 97, 0, 0, 0, 0, 0}).String(),
	)
}

func TestHeader_MarshalJSON(t *testing.T) {
	b, err := (&H{100, Test, TestRequest, 99, 98, 97, 0, 0, 0, 0, 0}).MarshalJSON()
	assert.Nil(t, err)
	assert.Equal(
		t,
