...
const (
	...
	RelayPacket   MessageType = 8
	NonTunMessage MessageType = 9
	DirectPingReq MessageType = 10
	DirectPingRep MessageType = 11
)
...
var typeMap = map[MessageType]string{
	...
	NonTunMessage: "nonTunMessage",
	DirectPingReq: "directPingReq",
	DirectPingRep: "directPingRep",
}
...
const (
	NonTunMessageMain MessageSubType = 0
	NonTunMessageACK  MessageSubType = 1
)
const (
...
var subTypeMap = map[MessageType]*map[MessageSubType]string{
	...
	RelayPacket:   &subTypeNoneMap,
	NonTunMessage: &subTypeNoneMap,
}
...
type H struct {
	...
	// destIP and sourceIP are used only when relaying
	DestIP     uint32
	SourceIP   uint32
	DestPort   uint16
	SourcePort uint16
...
func Encode(b []byte, v uint8, t MessageType, st MessageSubType, ri uint32, c uint64,
	destIP uint32, sourceIP uint32,
	destPort uint16, sourcePort uint16,
	networkID uint64) []byte {
	b = b[:Len]
	b[0] = v<<4 | byte(t&0x0f)
	b[1] = byte(st)
	...
	binary.BigEndian.PutUint32(b[16:20], destIP)
	binary.BigEndian.PutUint32(b[20:24], sourceIP)
	binary.BigEndian.PutUint16(b[24:26], destPort)
	binary.BigEndian.PutUint16(b[26:28], sourcePort)
	binary.BigEndian.PutUint64(b[28:36], networkID)
	return b
}
...
func (h *H) Encode(b []byte) ([]byte, error) {
	...
	return Encode(b, h.Version, h.Type, h.Subtype, h.RemoteIndex, h.MessageCounter, 0, 0, 0, 0, 0), nil
}
...
func (h *H) Parse(b []byte) error {
	...
	h.DestIP = binary.BigEndian.Uint32(b[16:20])
	h.SourceIP = binary.BigEndian.Uint32(b[20:24])
	h.DestPort = binary.BigEndian.Uint16(b[24:26])
	h.SourcePort = binary.BigEndian.Uint16(b[26:28])
	h.NetworkID = binary.BigEndian.Uint64(b[28:36])
	return nil
}
