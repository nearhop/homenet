...
func TestSystemTimerWheel_Add(t *testing.T) {
	...
	tw.Add(nip, time.Second*1)

	// Make sure we set head and tail properly
	assert.NotNil(t, tw.wheel[2])
	assert.Equal(t, nip, tw.wheel[2].Head.Item)
	assert.Nil(t, tw.wheel[2].Head.Next)
	assert.Equal(t, nip, tw.wheel[2].Tail.Item)
	...
	tw.Add(nip2, time.Second*1)
	assert.Equal(t, nip2, tw.wheel[2].Head.Item)
	assert.Equal(t, nip, tw.wheel[2].Head.Next.Item)
	assert.Equal(t, nip, tw.wheel[2].Tail.Item)
	assert.Nil(t, tw.wheel[2].Tail.Next)
...
func TestSystemTimerWheel_Purge(t *testing.T) {
	...
	nips := []NetworkIPPair{{9, 0}, {10, 0}, {11, 0}, {12, 0}}

	tw.Add(nips[0], time.Second*1)
	tw.Add(nips[1], time.Second*1)
	tw.Add(nips[2], time.Second*2)
	tw.Add(nips[3], time.Second*2)
	...
	for i := 0; i < 4; i++ {
		assert.Contains(t, nips, tw.Purge())
	}
