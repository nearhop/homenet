...
func Test_lhStaticMapping(t *testing.T) {
	...
	meh := NewLightHouse(l, true, &net.IPNet{IP: net.IP{0, 0, 0, 1}, Mask: net.IPMask{255, 255, 255, 255}}, []iputil.VpnIp{iputil.Ip2VpnIp(lh1IP)}, 10, 10003, udpServer, false, 1, false, 0)
	meh.AddStaticRemote(iputil.Ip2VpnIp(lh1IP), udp.NewAddr(lh1IP, uint16(4242)), 0)
	...
	meh = NewLightHouse(l, true, &net.IPNet{IP: net.IP{0, 0, 0, 1}, Mask: net.IPMask{255, 255, 255, 255}}, []iputil.VpnIp{iputil.Ip2VpnIp(lh1IP), iputil.Ip2VpnIp(lh2IP)}, 10, 10003, udpServer, false, 1, false, 0)
	meh.AddStaticRemote(iputil.Ip2VpnIp(lh1IP), udp.NewAddr(lh1IP, uint16(4242)), 0)
	err = meh.ValidateLHStaticEntries()
...
func BenchmarkLighthouseHandleRequest(b *testing.B) {
	...
	lh := NewLightHouse(l, true, &net.IPNet{IP: net.IP{0, 0, 0, 1}, Mask: net.IPMask{0, 0, 0, 0}}, []iputil.VpnIp{iputil.Ip2VpnIp(lh1IP)}, 10, 10003, udpServer, false, 1, false, 0)

	hAddr := udp.NewAddrFromString("4.5.6.7:12345")
	hAddr2 := udp.NewAddrFromString("4.5.6.7:12346")
	lh.addrMap[0][3] = NewRemoteList()
	lh.addrMap[0][3].unlockedSetV4(
		3,
		3,
		[]*Ip4AndPort{
	...
	lh.addrMap[0][2] = NewRemoteList()
	lh.addrMap[0][2].unlockedSetV4(
		3,
		3,
		[]*Ip4AndPort{
	...
	b.Run("notfound", func(b *testing.B) {
		...
		for n := 0; n < b.N; n++ {
			lhh.HandleRequest(rAddr, 2, 0, p, mw)
		}
	})
...
func TestLighthouse_Memory(t *testing.T) {
	...
	udpServer, _ := udp.NewListener(l, "0.0.0.0", 0, true, 2)
	lh := NewLightHouse(l, true, &net.IPNet{IP: net.IP{10, 128, 0, 1}, Mask: net.IPMask{255, 255, 255, 0}}, []iputil.VpnIp{}, 10, 10003, udpServer, false, 1, false, 0)
	lhh := lh.NewRequestHandler()
...
func newLHHostRequest(fromAddr *udp.Addr, myVpnIp, queryVpnIp iputil.VpnIp, lhh *LightHouseHandler) testLhReply {
	...
	w := &testEncWriter{}
	lhh.HandleRequest(fromAddr, myVpnIp, 0, b, w)
	return w.lastReply
}
...
func newLHHostUpdate(fromAddr *udp.Addr, vpnIp iputil.VpnIp, addrs []*udp.Addr, lhh *LightHouseHandler) {
	...
	w := &testEncWriter{}
	lhh.HandleRequest(fromAddr, vpnIp, 0, b, w)
}
...
func (tw *testEncWriter) SendMessageToVpnIp(t header.MessageType, st header.MessageSubType, vpnIp iputil.VpnIp, p, _, _ []byte, networkID uint64) {
...
func (tw *testEncWriter) SendRelay(t header.MessageType, st header.MessageSubType, p, nb, out []byte, destIP uint32, sourceIP uint32, destPort uint16, sourcePort uint16, networkID uint64, relayIP *iputil.VpnIp) {
	return
}
