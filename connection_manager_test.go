...
func Test_NewConnectionManagerTest(t *testing.T) {
	...
	lh := NewLightHouse(l, false, &net.IPNet{IP: net.IP{0, 0, 0, 0}, Mask: net.IPMask{0, 0, 0, 0}}, []iputil.VpnIp{}, 1000, 0, &udp.Conn{}, false, 1, false, 0)
	ifce := &Interface{
		hostMap:          hostMap,
		inside:           &test.NoopTun{},
		outside:          &udp.Conn{},
		...
		firewall:         &Firewall{},
		lightHouse:       lh,
		handshakeManager: NewHandshakeManager(l, vpncidr, preferredRanges, hostMap, lh, &udp.Conn{}, defaultHandshakeConfig, 0),
		l:                l,
	}
	now := time.Now()
	...
	hostinfo, _ := nc.hostMap.AddVpnIp(vpnIp, 0, nil)
	...
	assert.Contains(t, nc.hostMap.Hosts[0], vpnIp)
	...
	assert.NotContains(t, nc.pendingDeletion, vpnIp)
	assert.NotContains(t, nc.hostMap.Hosts[0], vpnIp)
