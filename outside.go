...
func (f *Interface) readOutsidePackets(addr *udp.Addr, out []byte, packet []byte, h *header.H, fwPacket *firewall.Packet, lhf udp.LightHouseHandlerFunc, nb []byte, q int, localCache firewall.ConntrackCache) {
	...
	if f.lightHouse.amLighthouse && h.NetworkID == 0 {
		// Drop packets without valid networkid
		return
	}

	//l.Error("in packet ", header, packet[HeaderLen:])
	// verify if we've seen this index before, otherwise respond to the handshake initiation
	hostinfo, err := f.hostMap.QueryIndex(h.RemoteIndex, h.NetworkID)

	var ci *ConnectionState
	if err == nil {
		ci = hostinfo.ConnectionState
	} else {
		f.l.Infof("err is not nil")
	}
	if hostinfo == nil {
		f.l.WithField("h.RemoteIndex", h.RemoteIndex).WithField("h.networkID", h.NetworkID).WithField("f.lightHouse.amLighthouse...", f.lightHouse.amLighthouse).WithField("h.SourceIP..", nh_util.Int2ip(h.SourceIP)).WithField("h.DestIP..", nh_util.Int2ip(h.DestIP)).WithField("f.networkID", f.networkID).WithField("header.Type", h.Type).WithField("header.SubType", h.Subtype).WithField("header.Version", h.Version).Infof("readoutsidepackets : hostinfo is nil")
	}
	...
	case header.Message:
		...
		hostinfo.in_bytes += (uint64)(len(packet))
	...
	case header.LightHouse:
		...
		lhf(addr, hostinfo.vpnIp, h.NetworkID, d, f)
	...
	case header.Handshake:
		f.messageMetrics.Rx(h.Type, h.Subtype, 1)
		HandleIncomingHandshake(f, addr, packet, h, hostinfo, 0, nil)
		return
	...
	case header.CloseTunnel:
		...
		f.closeTunnel(hostinfo, false, f.networkID)
		return

	case header.RelayPacket:
		if err == nil && hostinfo != nil {
			HandleRelay(f, addr, out, packet, h, hostinfo, nb, q, localCache)
		}
		return
	case header.NonTunMessage:
		...
		go f.messaging.recvMessage(hostinfo.vpnIp, dcopy)
		return
	case header.DirectPingReq:
		if hostinfo != nil {
			f.handleDirectPingReq(hostinfo, addr)
		}
		return
	case header.DirectPingRep:
		if hostinfo != nil {
			f.handleDirectPingRep(hostinfo, addr)
		}
		return
	default:
		f.messageMetrics.Rx(h.Type, h.Subtype, 1)
...
func (f *Interface) closeTunnel(hostInfo *HostInfo, hasHostMapLock bool, networkID uint64) {
	//TODO: this would be better as a single function in ConnectionManager that handled locks appropriately
	f.connectionManager.ClearIP(hostInfo.vpnIp)
	f.connectionManager.ClearPendingDeletion(hostInfo.vpnIp)
	f.lightHouse.DeleteVpnIp(hostInfo.vpnIp, networkID)
...
func (f *Interface) decryptToTun(hostinfo *HostInfo, messageCounter uint64, out []byte, packet []byte, fwPacket *firewall.Packet, nb []byte, q int, localCache firewall.ConntrackCache) {
	...
	/*
		if f.caPool[hostinfo.networkID] != nil {
			dropReason := f.firewall.Drop(out, *fwPacket, true, hostinfo, f.caPool[hostinfo.networkID], localCache)
			if dropReason != nil {
				if f.l.Level >= logrus.DebugLevel {
					hostinfo.logger(f.l).WithField("fwPacket", fwPacket).
...
func (f *Interface) sendRecvError(endpoint *udp.Addr, index uint32) {
	...
	b := header.Encode(make([]byte, header.Len), header.Version, header.RecvError, 0, index, 0, 0, 0, 0, 0, f.networkID)
...
func (f *Interface) handleRecvError(addr *udp.Addr, h *header.H) {
	...
	f.handshakeManager.pendingHostMap.DeleteReverseIndex(h.RemoteIndex, h.NetworkID)

	hostinfo, err := f.hostMap.QueryReverseIndex(h.RemoteIndex, h.NetworkID)
	if err != nil {
...
func (f *Interface) sendMeta(ci *ConnectionState, endpoint *net.UDPAddr, meta *NebulaMeta) {
	...
	c := ci.messageCounter
	b := HeaderEncode(nil, Version, uint8(metadata), 0, hostinfo.remoteIndexId, c, 0, 0, 0, 0, 0)
	ci.messageCounter++
