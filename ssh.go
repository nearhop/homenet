...
func attachCommands(l *logrus.Logger, ssh *sshd.SSHServer, hostMap *HostMap, pendingHostMap *HostMap, lightHouse *LightHouse, ifce *Interface) {
	ssh.RegisterCommand(&sshd.Command{
		...
		Callback: func(fs interface{}, a []string, w sshd.StringWriter) error {
			return sshListHostMap(hostMap, fs, a, w)
		},
	})
	...
	ssh.RegisterCommand(&sshd.Command{
		...
		Callback: func(fs interface{}, a []string, w sshd.StringWriter) error {
			return sshListHostMap(pendingHostMap, fs, a, w)
		},
	})
	...
	ssh.RegisterCommand(&sshd.Command{
		...
		Callback: func(fs interface{}, a []string, w sshd.StringWriter) error {
			return sshListLighthouseMap(lightHouse, fs, a, w)
		},
	})
	...
	ssh.RegisterCommand(&sshd.Command{
		Name:             "send-non-tun-message",
		ShortDescription: "Send a non-tun message to the provided VPN IP",
		Callback: func(fs interface{}, a []string, w sshd.StringWriter) error {
			return sshSendNonTunMessage(ifce, fs, a, w)
		},
	})
}

func sshSendNonTunMessage(ifce *Interface, fs interface{}, a []string, w sshd.StringWriter) error {
	if len(a) == 0 {
		return w.WriteLine("Insufficient number of arguments provided")
	}
	...
	ret, err := ifce.messaging.sendMessage(vpnIp, ifce.networkID, []byte(message), header.NonTunMessageMain, 0)
	if err != nil {
		fmt.Println("Error...", err.Error())
	}
	fmt.Println("Received message...", ret)
	return nil
}

func sshListHostMap(hostMap *HostMap, a interface{}, args []string, w sshd.StringWriter) error {
	...
	if len(args) == 0 {
		return w.WriteLine("No networkid was provided")
	}
	networkID, err := strconv.ParseUint(args[0], 10, 64)
	if err != nil {
		return w.WriteLine(fmt.Sprintf("The provided networkid could not be parsed: %s", args[0]))
	}

	hm := listHostMap(hostMap, networkID)
...
func sshListLighthouseMap(lightHouse *LightHouse, a interface{}, args []string, w sshd.StringWriter) error {
	fs, ok := a.(*sshListHostMapFlags)
	if !ok {
		//TODO: error
		return nil
	...
	lightHouse.RLock()
	addrMap := make([]lighthouseInfo, len(lightHouse.addrMap[networkID]))
	x := 0
	for k, v := range lightHouse.addrMap[networkID] {
...
func sshQueryLighthouse(ifce *Interface, fs interface{}, a []string, w sshd.StringWriter) error {
	if len(a) != 2 {
		return w.WriteLine("Need VPN IP and network id")
	}
	...
	var cm *CacheMap
	networkID, err := strconv.ParseUint(a[1], 10, 64)
	if err != nil {
		return w.WriteLine(fmt.Sprintf("The provided networkid could not be parsed: %s", a[1]))
	}
	rl := ifce.lightHouse.Query(vpnIp, networkID, ifce)
	if rl != nil {
		cm = rl.CopyCache()
	}
...
func sshCloseTunnel(ifce *Interface, fs interface{}, a []string, w sshd.StringWriter) error {
	...
	if len(a) != 2 {
		return w.WriteLine("Both VPN IP and network id were needed")
	}
	...
	hostInfo, err := ifce.hostMap.QueryVpnIp(vpnIp, networkID)
	if err != nil {
	...
	ifce.closeTunnel(hostInfo, false, 0)
...
func sshCreateTunnel(ifce *Interface, fs interface{}, a []string, w sshd.StringWriter) error {
	...
	if len(a) != 2 {
		return w.WriteLine("VPN IP and networkid need to be provided")
	}
	...
	hostInfo, _ := ifce.hostMap.QueryVpnIp(vpnIp, networkID)
	if hostInfo != nil {
		return w.WriteLine(fmt.Sprintf("Tunnel already exists"))
	}

	hostInfo, _ = ifce.handshakeManager.pendingHostMap.QueryVpnIp(vpnIp, networkID)
	if hostInfo != nil {
	...
	hostInfo = ifce.handshakeManager.AddVpnIp(vpnIp, networkID, ifce.initHostInfo)
	if addr != nil {
		hostInfo.SetRemote(addr)
	}
	ifce.getOrHandshake(vpnIp, networkID, true)
...
func sshChangeRemote(ifce *Interface, fs interface{}, a []string, w sshd.StringWriter) error {
	...
	if len(a) != 2 {
		return w.WriteLine("We need VPN IP and networkID")
	}
...
func sshPrintTunnel(ifce *Interface, fs interface{}, a []string, w sshd.StringWriter) error {
	...
	if len(a) != 2 {
		return w.WriteLine("NetworkID and vpn ip are needed")
	}
