...
type HostMap struct {
	...
	name            string
	Indexes         map[uint64]map[uint32]*HostInfo
	RemoteIndexes   map[uint64]map[uint32]*HostInfo
	Hosts           map[uint64]map[iputil.VpnIp]*HostInfo
...
func NewHostMap(l *logrus.Logger, name string, vpnCIDR *net.IPNet, preferredRanges []*net.IPNet) *HostMap {
	l.WithField("name ", name).Error("Hostmap created")
	m := HostMap{
		name:            name,
		Indexes:         make(map[uint64]map[uint32]*HostInfo),
		RemoteIndexes:   make(map[uint64]map[uint32]*HostInfo),
		Hosts:           make(map[uint64]map[iputil.VpnIp]*HostInfo),
...
func (hm *HostMap) GetIndexByVpnIp(vpnIp iputil.VpnIp, networkID uint64) (uint32, error) {
	hm.RLock()
	if i, ok := hm.Hosts[networkID][vpnIp]; ok {
...
func (hm *HostMap) Add(ip iputil.VpnIp, hostinfo *HostInfo, networkID uint64) {
	hm.Lock()
	if hm.Hosts[networkID] == nil {
		hm.Hosts[networkID] = make(map[iputil.VpnIp]*HostInfo)
	}
	hm.Hosts[networkID][ip] = hostinfo
	hm.Unlock()
}

func (hm *HostMap) AddVpnIp(vpnIp iputil.VpnIp, networkID uint64, init func(hostinfo *HostInfo)) (hostinfo *HostInfo, created bool) {
	hm.RLock()
	if h, ok := hm.Hosts[networkID][vpnIp]; !ok {
		hm.RUnlock()
		h = &HostInfo{
			promoteCounter:  0,
			vpnIp:           vpnIp,
		...
		hm.Lock()
		hm.l.WithField("vpnIp ", vpnIp).WithField("networkID", networkID).Error("2. Added VPN IP")
		if hm.Hosts[networkID] == nil {
			hm.Hosts[networkID] = make(map[iputil.VpnIp]*HostInfo)
		}
		hm.Hosts[networkID][vpnIp] = h
		hm.Unlock()
		return h, true
	} else {
		hm.RUnlock()
...
func (hm *HostMap) DeleteVpnIp(vpnIp iputil.VpnIp, networkID uint64) {
	hm.Lock()
	delete(hm.Hosts[networkID], vpnIp)
	if len(hm.Hosts[networkID]) == 0 {
		hm.Hosts[networkID] = map[iputil.VpnIp]*HostInfo{}
	}
	hm.Unlock()
...
func (hm *HostMap) addRemoteIndexHostInfo(index uint32, h *HostInfo, networkID uint64) {
	hm.Lock()
	h.remoteIndexId = index
	hm.RemoteIndexes[networkID][index] = h
	hm.Unlock()
...
func (hm *HostMap) AddVpnIpHostInfo(vpnIp iputil.VpnIp, h *HostInfo, networkID uint64) {
	hm.Lock()
	h.vpnIp = vpnIp
	hm.Hosts[networkID][vpnIp] = h
	hm.Indexes[networkID][h.localIndexId] = h
	hm.RemoteIndexes[networkID][h.remoteIndexId] = h
	hm.Unlock()
	hm.l.WithField("vpnIP", vpnIp).WithField("networkID", networkID).WithField("h.remoteIndexId", h.remoteIndexId).WithField("h.localIndexId", h.localIndexId).Error("3. Added VPN Ip hostinfo")
}

// This is only called in pendingHostmap, to cleanup an inbound handshake
func (hm *HostMap) DeleteIndex(index uint32, networkID uint64) {
	hm.Lock()
	hostinfo, ok := hm.Indexes[networkID][index]
	if ok {
		delete(hm.Indexes[networkID], index)
		delete(hm.RemoteIndexes[networkID], hostinfo.remoteIndexId)

		// Check if we have an entry under hostId that matches the same hostinfo
		// instance. Clean it up as well if we do.
		hostinfo2, ok := hm.Hosts[networkID][hostinfo.vpnIp]
		if ok && hostinfo2 == hostinfo {
			delete(hm.Hosts[networkID], hostinfo.vpnIp)
		}
	}
	hm.Unlock()
}
...
func (hm *HostMap) DeleteReverseIndex(index uint32, networkID uint64) {
	hm.Lock()
	hostinfo, ok := hm.RemoteIndexes[networkID][index]
	if ok {
		delete(hm.Indexes[networkID], hostinfo.localIndexId)
		delete(hm.RemoteIndexes[networkID], index)
...
func (hm *HostMap) unlockedDeleteHostInfo(hostinfo *HostInfo) {
	...
	hostinfo2, ok := hm.Hosts[hostinfo.networkID][hostinfo.vpnIp]
	if ok && hostinfo2 != hostinfo {
		delete(hm.Hosts[hostinfo.networkID], hostinfo2.vpnIp)
		delete(hm.Indexes[hostinfo.networkID], hostinfo2.localIndexId)
		delete(hm.RemoteIndexes[hostinfo.networkID], hostinfo2.remoteIndexId)
	}

	delete(hm.Hosts[hostinfo.networkID], hostinfo.vpnIp)
	if len(hm.Hosts[hostinfo.networkID]) == 0 {
		hm.Hosts[hostinfo.networkID] = map[iputil.VpnIp]*HostInfo{}
	}
	delete(hm.Indexes[hostinfo.networkID], hostinfo.localIndexId)
	if len(hm.Indexes[hostinfo.networkID]) == 0 {
		hm.Indexes[hostinfo.networkID] = map[uint32]*HostInfo{}
	}
	delete(hm.RemoteIndexes[hostinfo.networkID], hostinfo.remoteIndexId)
	if len(hm.RemoteIndexes[hostinfo.networkID]) == 0 {
		hm.RemoteIndexes[hostinfo.networkID] = map[uint32]*HostInfo{}
	}

	if hm.l.Level >= logrus.DebugLevel {
		hm.l.WithField("hostMap", m{"mapName": hm.name, "mapTotalSize": len(hm.Hosts[hostinfo.networkID]),
			"vpnIp": hostinfo.vpnIp, "indexNumber": hostinfo.localIndexId, "remoteIndexNumber": hostinfo.remoteIndexId}).
			Debug("Hostmap hostInfo deleted")
	}
}

func (hm *HostMap) QueryIndex(index uint32, networkID uint64) (*HostInfo, error) {
	//TODO: we probably just want ot return bool instead of error, or at least a static error
	hm.RLock()
	if h, ok := hm.Indexes[networkID][index]; ok {
		hm.RUnlock()
		return h, nil
	} else {
		hm.RUnlock()
...
func (hm *HostMap) QueryReverseIndex(index uint32, networkID uint64) (*HostInfo, error) {
	hm.RLock()
	if h, ok := hm.RemoteIndexes[networkID][index]; ok {
		hm.RUnlock()
		return h, nil
	} else {
		hm.RUnlock()
...
func (hm *HostMap) QueryVpnIp(vpnIp iputil.VpnIp, networkID uint64) (*HostInfo, error) {
	return hm.queryVpnIp(vpnIp, nil, networkID)
}
...
func (hm *HostMap) PromoteBestQueryVpnIp(vpnIp iputil.VpnIp, ifce *Interface, networkID uint64) (*HostInfo, error) {
	return hm.queryVpnIp(vpnIp, ifce, networkID)
}

func (hm *HostMap) queryVpnIp(vpnIp iputil.VpnIp, promoteIfce *Interface, networkID uint64) (*HostInfo, error) {
	hm.RLock()
	if h, ok := hm.Hosts[networkID][vpnIp]; ok {
		hm.RUnlock()
...
func (hm *HostMap) addHostInfo(hostinfo *HostInfo, f *Interface) {
	...
	if hm.Hosts[hostinfo.networkID] == nil {
		hm.Hosts[hostinfo.networkID] = make(map[iputil.VpnIp]*HostInfo)
	}
	hm.Hosts[hostinfo.networkID][hostinfo.vpnIp] = hostinfo
	if hm.Indexes[hostinfo.networkID] == nil {
		hm.Indexes[hostinfo.networkID] = make(map[uint32]*HostInfo)
	}
	hm.l.WithField("vpnIP", hostinfo.vpnIp).WithField("h.remoteIndexId", hostinfo.remoteIndexId).WithField("h.localIndexId", hostinfo.localIndexId).WithField("hostinfo.networkID", hostinfo.networkID).Error("12. Added VPN Ip hostinfo")
	hm.Indexes[hostinfo.networkID][hostinfo.localIndexId] = hostinfo
	if hm.RemoteIndexes[hostinfo.networkID] == nil {
		hm.RemoteIndexes[hostinfo.networkID] = make(map[uint32]*HostInfo)
	}
	hm.RemoteIndexes[hostinfo.networkID][hostinfo.remoteIndexId] = hostinfo

	if hm.l.Level >= logrus.DebugLevel {
		hm.l.WithField("hostMap", m{"mapName": hm.name, "vpnIp": hostinfo.vpnIp, "mapTotalSize": len(hm.Hosts[hostinfo.networkID]),
...
func (i *HostInfo) TryPromoteBest(preferredRanges []*net.IPNet, ifce *Interface) {
	...
	if c%ReQueryEvery == 0 && ifce.lightHouse != nil {
		ifce.lightHouse.QueryServer(i.vpnIp, ifce.networkID, ifce)
	}
}
...
func (i *HostInfo) getRemoteIPs(tunCidr *net.IPNet) []net.IP {
	ips := make([]net.IP, len(i.remotes.addrs))

	ipindex := 0
	for index, _ := range i.remotes.addrs {
		if tunCidr.Contains(i.remotes.addrs[index].IP) {
			continue
		}
		if ipindex > len(ips) {
			// Not sure why. We are hitting into array index out of bound issue
			// When VPN is turned off on mobile app
			break
		}
		ips[ipindex] = i.remotes.addrs[index].IP
		ipindex++
	}
	return ips[0:ipindex]
}
