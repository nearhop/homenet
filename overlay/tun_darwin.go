...
func newTun(l *logrus.Logger, name string, cidr *net.IPNet, defaultMTU int, routes []Route, _ int, _ bool, dns []net.IP) (*tun, error) {
...
func newTunFromFd(_ *logrus.Logger, _ int, _ *net.IPNet, _ int, _ []Route, _ int, dns []net.IP) (*tun, error) {
...
func (t tun) AddRoutes(Routes []Route) error {
	var err error
	if t.routeSock == -1 {
		t.routeSock, err = unix.Socket(unix.AF_ROUTE, unix.SOCK_RAW, unix.AF_UNSPEC)
		if err != nil {
			return fmt.Errorf("unable to create AF_ROUTE socket: %v", err)
		}
		defer func() {
			unix.Shutdown(t.routeSock, unix.SHUT_RDWR)
			err := unix.Close(t.routeSock)
			if err != nil {
	...
	for _, r := range Routes {
		if r.Via == nil && r.GW == nil {
			// No Via and No GW.
			continue
		}

		flags := unix.RTF_UP
		var link netroute.Addr
		link = linkAddr
		if r.GW != nil && r.Iname != "" {
			flags = (unix.RTF_UP | unix.RTF_GATEWAY | unix.RTF_STATIC | unix.RTF_PRCLONING)
			ipString := r.GW.String()
			...
			link1 := &netroute.Inet4Addr{}
			link1.IP[0] = byte(octet0)
			link1.IP[1] = byte(octet1)
			link1.IP[2] = byte(octet2)
			link1.IP[3] = byte(octet3)

			link = link1
		}
		copy(routeAddr.IP[:], r.Cidr.IP.To4())
		copy(maskAddr.IP[:], net.IP(r.Cidr.Mask).To4())
		err := addRoute(t.routeSock, routeAddr, maskAddr, link, flags)
		if err != nil {
...
func (t *tun) Activate() error {
	...
	*/
	...
	err = addRoute(t.routeSock, routeAddr, maskAddr, linkAddr, unix.RTF_UP)
	if err != nil {
		if errors.Is(err, unix.EEXIST) {
	...
	err = t.AddRoutes(t.Routes)
	if err != nil {
		return err
	}

	if t.DNSServers != nil {
		nameserver := t.DNSServers[0].String()
		for i, server := range t.DNSServers {
			if i == 0 {
				continue
			}
			nameserver = nameserver + " " + server.String()
		}
		t.l.WithField("Nameserver1..", nameserver).Error("DNS Servers")
		cmd := "-setdnsservers Wi-Fi " + nameserver
		cmdstring := strings.Split(cmd, " ")
		err := exec.Command("networksetup", cmdstring...).Run()
		if err != nil {
			return fmt.Errorf("Error while setting up DNS Servers", err)
		}
	}
...
func addRouteInternal(sock int, addr, mask *netroute.Inet4Addr, link netroute.Addr, flags int, rtype int) error {
...
func addRoute(sock int, addr, mask *netroute.Inet4Addr, link netroute.Addr, flags int) error {
	addRouteInternal(sock, addr, mask, link, flags, unix.RTM_DELETE)
	return addRouteInternal(sock, addr, mask, link, flags, unix.RTM_ADD)
}
