...
func newTunFromFd(_ *logrus.Logger, _ int, _ *net.IPNet, _ int, _ []Route, _ int, dns []net.IP) (Device, error) {
	return nil, fmt.Errorf("newTunFromFd not supported in Windows")
}

func newTun(l *logrus.Logger, deviceName string, cidr *net.IPNet, defaultMTU int, routes []Route, _ int, _ bool, dns []net.IP) (Device, error) {
	l.Error("routes length...", len(routes))
	...
	if useWintun {
		device, err := newWinTun(l, deviceName, cidr, defaultMTU, routes, dns)
		if err != nil {
	...
	device, err := newWaterTun(l, cidr, defaultMTU, routes, dns)
	if err != nil {
