...
func (n *connectionManager) Out(ip iputil.VpnIp, networkID uint64) {
	n.outLock.RLock()
	// If this already exists, return
	if _, ok := n.out[ip]; ok {
		n.outLock.RUnlock()
	...
	n.AddTrafficWatch(nip, n.checkInterval)
	n.outLock.Unlock()
}
...
func (n *connectionManager) AddPendingDeletion(ip iputil.VpnIp, networkID uint64) {
	n.pendingDeletionLock.Lock()
	...
	n.pendingDeletionTimer.Add(nip, time.Second*time.Duration(n.pendingDeletionInterval))
	n.pendingDeletionLock.Unlock()
}
...
func (n *connectionManager) AddTrafficWatch(nip NetworkIPPair, seconds int) {
	n.TrafficTimer.Add(nip, time.Second*time.Duration(seconds))
}
...
func (n *connectionManager) HandleMonitorTick(now time.Time, p, nb, out []byte) {
	...
	for {
		...
		vpnIp := nip.vpnIP
		networkID := nip.networkID

		// Check for traffic coming back in from this host.
		traf := n.CheckIn(vpnIp)

		hostinfo, err := n.hostMap.QueryVpnIp(vpnIp, networkID)
		if err != nil {
		...
		if hostinfo != nil && hostinfo.ConnectionState != nil {
			...
			n.intf.SendMessageToVpnIp(header.Test, header.TestRequest, vpnIp, p, nb, out, networkID)

		} else {
			hostinfo.logger(n.l).Debugf("Hostinfo sadness: %s", vpnIp)
		}
		n.AddPendingDeletion(vpnIp, networkID)
	}
	...
	for {
		...
		if n.checkPendingDeletion(vpnIp) {
			...
			hostinfo.logger(n.l).
				...
				Info("2. Tunnel status")

			n.ClearIP(vpnIp)
			n.ClearPendingDeletion(vpnIp)
			// TODO: This is only here to let tests work. Should do proper mocking
			if n.intf.lightHouse != nil {
				n.intf.lightHouse.DeleteVpnIp(vpnIp, n.intf.networkID)
			}
			n.hostMap.DeleteHostInfo(hostinfo)
			n.intf.UpdateRelayHostInfo()
		} else {
			n.ClearIP(vpnIp)
			n.ClearPendingDeletion(vpnIp)
		}
...
func (n *connectionManager) handleInvalidCertificate(now time.Time, vpnIp iputil.VpnIp, hostinfo *HostInfo) bool {
	...
	if n.intf.lightHouse.amLighthouse {
		ca, _, _, err = get_network_certs(hostinfo.networkID, n.intf.sqlsecret)
		if err != nil {
			n.l.WithError(err).WithField("vpnip", vpnIp).
				WithField("networkID", hostinfo.networkID).
				Error("No Ca for this network")
		}
	} else {
		ca = n.intf.caFile
	}
	caPool, err := loadCAFromFile(n.intf.l, ca)
	if err != nil {
		n.l.WithError(err).WithField("vpnIp", hostinfo.vpnIp).
			WithField("networkID", hostinfo.networkID).
			Error("Unable to load CA")
		return false
	}
	valid, err := remoteCert.Verify(now, caPool)
	if valid {
		return false
	}
	...
	n.intf.closeTunnel(hostinfo, false, n.intf.networkID)
