...
func Test_NewHandshakeManagerVpnIp(t *testing.T) {
	...
	blah := NewHandshakeManager(l, tuncidr, preferredRanges, mainHM, &LightHouse{}, &udp.Conn{}, defaultHandshakeConfig, 0)
	...
	i := blah.AddVpnIp(ip, 0, initFunc)
	assert.True(t, initCalled)

	initCalled = false
	i2 := blah.AddVpnIp(ip, 0, initFunc)
	...
	assert.Contains(t, blah.pendingHostMap.Hosts[0], ip)
...
func Test_NewHandshakeManagerTrigger(t *testing.T) {
	...
	mw := &mockEncWriter{}
	mainHM := NewHostMap(l, "test", vpncidr, preferredRanges)
	lh := &LightHouse{addrMap: make(map[uint64]map[iputil.VpnIp]*RemoteList), l: l}

	blah := NewHandshakeManager(l, tuncidr, preferredRanges, mainHM, lh, &udp.Conn{}, defaultHandshakeConfig, 0)
	...
	blah.handleOutbound(ip, mw, true, 0)
...
func (mw *mockEncWriter) SendMessageToVpnIp(t header.MessageType, st header.MessageSubType, vpnIp iputil.VpnIp, p, nb, out []byte, networkID uint64) {
	return
}

func (mw *mockEncWriter) SendRelay(t header.MessageType, st header.MessageSubType, p, nb, out []byte, destIP uint32, sourceIP uint32, destPort uint16, sourcePort uint16, networkID uint64, relayIP *iputil.VpnIp) {
	return
}
